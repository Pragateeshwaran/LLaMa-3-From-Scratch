...
device = 'cpu'  # Set to CPU explicitly; uncomment below for GPU support
# device = 'cuda' if torch.cuda.is_available() else 'cpu'
# model = model.to(device)

# Configurations
max_epochs = 100000
max_lr = 3e-4           # As per Llama paper
min_lr = max_lr * 0.1   # As per Llama paper
warmup_steps = 2000     # As per Llama paper
weight_decay = 0.1      # As per Llama paper
Opti_Beta1 = 0.9        # As per Llama paper
Opti_Beta2 = 0.95       # Standard AdamW value
Opti_epi = 1e-5         # As per Llama paper
B = 16                  # Batch size (adjust based on memory)
T = 512                 # Sequence length (adjust based on memory)
...
print(f"Grad accumulation is {grad_accum_steps}")
...
for iter in range(max_epochs):
    ...
    train_lossi.append(loss_accum.item())
